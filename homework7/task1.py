...
class Segment:
    ...
    def __init__(self, coordinates1, coordinates2):
        self.first_coordinates = coordinates1
        self.second_coordinates = coordinates2

    def length(self):
        first_len = (self.second_coordinates[1] - self.first_coordinates[1]) ** 2
        second_len = (self.second_coordinates[0] - self.first_coordinates[0]) ** 2
        whole_len = round((math.sqrt(first_len + second_len)), 2)
        return whole_len

    def x_axis_intersection(self):
        if (self.first_coordinates[1] * self.second_coordinates[1]) <= 0:
            return True
        else:
            return False

    def y_axis_intersection(self):
        if (self.first_coordinates[0] * self.second_coordinates[0]) <= 0:
            return True
        else:
            return False
