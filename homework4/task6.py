...
def create_phone_number(num_tuple):
    tuple_to_stroke = ''.join(map(str, num_tuple))
    first_part = tuple_to_stroke[0:3]
    second_part = tuple_to_stroke[3:6]
    third_part = tuple_to_stroke[6:]
    str_phone = f'({first_part}) {second_part}-{third_part}'
    return str_phone
