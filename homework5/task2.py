...
def repeats(our_str):
    """
    Функция принимает на вход строку (our_str) и возвращает новую строку (new_str),
    добавляя к каждой букве _n, где 'n' - количество использования буквы в строке

    :param our_str: На вход подается строка
    :return: Возвращается новая строка
    """
    new_str = ''
    count_it = 0
    for letter in our_str:
        ...
        if letter in new_str:
            count_it = new_str.count(letter) + 1
        new_str += f'{letter}_{count_it}'
        count_it = 0
    return new_str
