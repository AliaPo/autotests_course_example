...
def everything_for_your_cat(cats_data):
    """
    Функция принимает на вход список кортежей (cats_data) и оптимизирует его так,
    чтобы данные обо всех животных одного владельца выводились в одну строку (our_str).

    :param cats_data: На вход подается список кортежей с данными о питомцах и их владельцах
    :return: Построчно возвращается информация о владельце и его питомцах
    """
    owner_and_pet = {}

    lst_of_pets = []

    for pet_name, pet_age, owner_name, owner_surname in cats_data:
        key = owner_name + ' ' + owner_surname
        value = pet_name + ', ' + str(pet_age)
        owner_and_pet.setdefault(key, lst_of_pets).append(value)
        lst_of_pets = []

    our_str = ''

    for owner, pet in owner_and_pet.items():
        first_part = owner + ': '
        second_part = '; '.join(pet)
        our_str += first_part + second_part + '\n'
